...
static const struct OpcodeDetails optab[88] = {
...
static const char size_arr[3] = {'B','W','L'};
...
static void sprintmode(unsigned int mode, unsigned int reg, unsigned int size, char *out_s) {
	...
	switch(mode) {
		...
		case 10 : {/* PC with index + disp */
			...
			if (mode == 6) {
				if (itype == 0) {
					sprintf(out_s, "%+i(A%u,D%i.%c)", displacement, reg, ireg, ir[isize]);
				} else {
					sprintf(out_s, "%+i(A%u,A%i.%c)", displacement, reg, ireg, ir[isize]);
				}
			} else { /* PC */
				if (itype == 0) {
		...
		for (int opnum = 1; opnum <= 87; ++opnum) {
			if ((word & optab[opnum].and) == optab[opnum].xor) {
				...
				switch(opnum) { /* opnum = 1..85 */
					...
					default : {
						sprintf(line_buf,"opnum out of range in switch (=%i)\n", opnum);
						print(0,line_buf);
						exit(1);
					}
				}
